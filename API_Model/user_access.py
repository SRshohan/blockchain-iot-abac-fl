...
class UserAccess(Resource):
    def post(self):
        try:
            ...
            user_associate_location = data.get("location")
            status = data.get("status")
            access_device = data.get("device_id")

            if not user_id or not user_associate_location or not status:
            ...
            identity_variables = {
                ...
                "CORE_PEER_MSPCONFIGPATH" : f"{network_directory}/organizations/peerOrganizations/org1.example.com/users/{user_id}@org1.example.com/msp",
            ...
            #Access device
            access_device = [
                "peer", "chaincode", "invoke", "mychannel", "abac", '{"function":"ConditionOfUserAccess", f"Args":[{access_device}]}'
            ]
            
            result = subprocess.run(access_device, cwd=network_directory, env=identity_variables, check=True)

            return jsonify({"message": "Command run successfully",

...
chaincodeList() {
  ...
  then
    ...
    [ "$1" = "peer2.org1.example.com" ]
  then

    peerChaincodeListTls "cli.org1.example.com" "peer2.org1.example.com:7043" "$2" "crypto-orderer/tlsca.orderer.example.com-cert.pem" # Third argument is channel name
...
chaincodeInvoke() {
  ...
  if [[ "$1" == *"peer2.org1.example.com"* ]]; then
    cli="cli.org1.example.com"
    peer_addresses="$peer_addresses,peer2.org1.example.com:7043"

    peer_certs="$peer_certs,crypto/peers/peer2.org1.example.com/tls/ca.crt"
